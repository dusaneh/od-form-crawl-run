...
export function classifyProtectedField({
  ...
  if (
    /credit|debit|card number|cvv|cvc|routing|bank account|payment/.test(
      fieldTokens,
    )
  ) {
  ...
  if (
    ...
    /agree|terms|consent|certif|authorize|signature|attest|acknowledge/.test(
      fieldTokens,
    )
  ) {
...
export function isConspicuouslySynthetic(value, field, fact = null) {
  ...
  if (field?.controlType === "email") return /@example\.invalid$/i.test(text);
  if (field?.controlType === "url") return /example\.invalid/i.test(text);
