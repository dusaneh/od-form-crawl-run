...
function promptText(observation) {
  return [
    ...
    "For every unprotected visible select, radio group, checkbox, or switch, propose explicit choice_probe actions covering every non-placeholder observed option. For a checkbox or switch, propose both false and true. Each choice_probe value must be an observed raw option value or boolean state; shared code will reject incomplete coverage and will not invent missing probes.",
    ...
    "A landing or introduction surface with no applicant controls may be a nonterminal form state when a visible application-entry action clearly advances into the form.",
    "Open every visible collapsed details, accordion, expando, disclosure, or aria-expanded control exactly once before unrelated progression or terminal submission, even when no applicant field is currently known to be inside it. The opened content may contain fields, guidance, eligibility language, consent terms, or submission criteria. Select the exact observed disclosure fact as an advance, then let the runtime re-sense the page; never repeat an already-expanded disclosure.",
    ...
    "The shared browser physics performs a fixed pointer sweep and bounded scrolling of the main document, child frames, and nested scroll containers before sensing to trigger legitimate hover, viewport, and lazy-load behavior. Do not invent pointer or scroll controls as form actions, and never describe this priming as CAPTCHA or bot-detection bypass.",
