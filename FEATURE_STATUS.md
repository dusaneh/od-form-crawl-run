...
| `F1.1–F1.3` rendered crawl/journey | **Partial** | Real Chromium, bounded same-origin discovery, cumulative multi-state journeys, iframes/open shadow roots, waits, and scrolling work. Mid-flow entry warning and locale handling remain. |
...
| `F1.6` obstacles | **Partial** | Stable waits plus required fixed pointer and bounded document/frame/nested-container scrolling are shared physics. Cookie gates and every visible collapsed disclosure are LLM-authored actions; collapsed expandos block unrelated progression until opened once and re-sensed, while already-expanded controls cannot be reused. Interactive CAPTCHA and required login produce explicit disqualified crawl/form eligibility; broader public proof remains. |
