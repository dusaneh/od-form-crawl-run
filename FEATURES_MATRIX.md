...
| Semantic/per-site actuator split | `F1.3.4 / F13 / F14` | **Partial — enforced default, parity pending** | Strict semantic commands, independently generated readable per-target handlers, deterministic per-site bundle assembly, source/capability validation, typed semantic and actuator repair, immutable preflight staging, PostgreSQL persistence, and browser preflight are implemented. Nonterminal progression is preflighted and the clean page is restored before the full run. Production defaults to enforced handlers; an enforced-corpus parity run remains. |
...
| Predictable obstacles | `F1.6` | **Partial** | Stable waits, a fixed pointer sweep, and bounded reversible document/frame scrolling are generic physics. Shared priming never clicks disclosures and leaves nested application scroll regions untouched; the observation exposes them to generated per-site handlers for verified scroll-gated sequences. Cookie gates and collapsed disclosures remain LLM-authored actions outside applicant fields. Complete broad public-site proof remains. |
...
| Evidence/provenance | `F2` | **Partial** | HTML, JSON, JSONL, model provenance, script plans, compact key-moment transition/result screenshots, upload readback, and result criteria persist. Routine model-sensing and successful option-probe images are transient. Phase 2 sensitive masking remains. |
...
| Cumulative evidence | `F2.2.12–F2.3.5` | **Built** | Each transition retains at most the completed base state, final selected branch, and resulting state; terminal and failure boundaries remain clickable. Exhaustive probe screenshots are not persisted. |
...
| Completion reporting | `F3.2 / F3.5 / F3.17` | **Built** | UI separates planned, attempted, verified, never-attempted, and attempted-failure field counts; API failures render all discoverable root causes in plain language alongside transport facts, rendered markers/confidence, provenance, and evidence. |
| API-console report presentation | `F3.19` | **Built** | Completed reports render crawl totals, the LLM-authored runner journey in human execution order, compact labeled key-moment evidence, and readable forms grouped by section with field types and critical flags; raw JSON remains inspectable. |
...
| Private operations dashboard | `F3.20 / F2.4.3–F2.4.4` | **Built** | An admin-only audit view provides pipeline/login totals, operational time/category/severity/user filtering, independent login history through five years, actor attribution, LLM timing, recent model calls, and an expandable timeline. Only `dbosmail@gmail.com` receives the admin role and dashboard link. |
...
| Hosted gateway | `F4.6.1` | **Built and deployed** | One public process serves the API landing page and assets, protects `/control-plane`, `/api-console`, and the admin dashboard, proxies `/api/*`, and runs API/UI children on loopback-only ports. |
...
| Admin dashboard authorization | `F4.6.8` | **Built** | Hosted dashboard access requires the persisted admin role in addition to an individual UI session; non-admin users, direct requests, and API Bearer tokens are denied server-side. |
| Hosted safety/storage | `F4.6.5–F4.6.7` | **Partial** | Hosted launch surfaces disable Headful and localhost controls, API bypass attempts are rejected, and bootstrap secrets are Git-ignored. PostgreSQL is durable, but screenshot/object storage and durable browser workers remain. |
...
| Unified operational audit | `F6.7` | **Built** | Append-only actor-aware events cover login, API failures/starts, crawl milestones and halts, approval, execution outcomes, and normalized LLM call timing/outcomes. Hosted records persist in PostgreSQL; local fallback is JSONL; prompts, values, screenshots, and secrets are excluded. |
...
| Automated tests | `F8` | **Partial** | Production build and 90 executable automated checks pass, with one optional PostgreSQL integration check skipped without its dedicated test URI. Authenticated production smoke and the blind corpus are green; public D5 remains. |
...
| Phase 1 generation loop | `F14` | **Partial** | Novel-state generation, typed semantic repairs, independently replaceable complete actuator handlers, cross-layer defect routing, immutable readable storage, branch expansion, validation replay, and retained zero-LLM replay work. Enforced-corpus parity and general N+1 lineage remain. |
...
| Execution-based drift | `F15` | **Partial** | Preflight/runtime faults route deterministically to semantic, actuator, environment, or drift suspicion, with bounded LLM diagnosis only when ambiguous. Re-crawl verdicts, staleness, and execution-aware version selection remain. |
