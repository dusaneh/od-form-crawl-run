...
| Private operations dashboard | `F3.20` | **Built** | An unlinked, authenticated audit view provides filtered pipeline/login totals, actor attribution, and an expandable diagnostic timeline. Bearer tokens, direct local requests, and caller-spoofed identity headers cannot retrieve shared audit data. |
...
| Unified operational audit | `F6.7` | **Built** | Append-only actor-aware events cover login, API failures/starts, crawl milestones and halts, approval, and execution outcomes. Hosted records persist in PostgreSQL; local fallback is JSONL; values and secrets are excluded. |
