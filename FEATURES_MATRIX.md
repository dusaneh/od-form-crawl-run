...
| API-console report presentation | `F3.19` | **Built** | Completed reports render crawl totals, compact labeled key-moment evidence, and readable forms grouped by section with field types and critical flags; raw JSON remains inspectable. |
...
| Automated tests | `F8` | **Partial** | Production build and 87 executable automated checks pass, with one optional PostgreSQL integration check skipped without its dedicated test URI. Authenticated production smoke and the blind corpus are green; public D5 remains. |
