...
| Same-page branching | `F1.7.9 / F16.6–F16.13` | **Built for one level** | Every safe observed choice is probed from a clean baseline, one reveal level is populated/verified, sibling variants are replayed, and the selected branch is restored before submit. A second reveal level halts. |
...
| Completion verification | `F2.2.11` | **Built** | A fixture success requires exact terminal actuation and explicit rendered markers. Normal forms also require submit/write transport; the loopback-only client-side exception requires a material state change and separately reports that basis. A GET to a confirmation-looking URL is insufficient. |
...
| Control-plane UI | `F3` | **Partial** | UI exposes runs, contracts, sections, guidance, option coverage, four-layer exchanges, script version/path/hash, fields, diagnostics, and clickable evidence. Certification/version browsing remain. |
...
| Local ownership | `F4` | **Built** | UI, API, Playwright, reports, logs, scripts, fixtures, and configuration run locally; only OpenAI model calls leave the machine. |
...
| Phase 1 safety | `F7` | **Partial** | Public terminal submit is browser-blocked and loopback fixture submit has separate authority. Interactive CAPTCHA and required login now explicitly disqualify; payment remains prohibited. Synthetic special-component modeling is origin-neutral; broader public proof remains. |
...
| Automated tests | `F8` | **Partial** | Production build and 74/74 serialized tests pass; local mechanics and the blind corpus are broad. Public D5 remains. |
...
| Phase 2 real-data runner | `F10 / F10.14` | **Partial — vertical slice verified** | Crawl returns typed schema; exact form approval, validated input, deterministic dry-run/submit execution, redacted logs, and structured result status work. Coverage certification, authentication, cancellation, public validation, and security hardening remain. |
...
| Sensitive evidence masking | `F12` | **Partial** | Input values/file bytes are not persisted, events redact values, and supplied scalar echoes/visible controls are redacted before post-submit model assessment. Comprehensive evidence leak tests and authenticated transport remain. |
