...
| Private operations dashboard | `F3.20 / F2.4.3–F2.4.4` | **Built** | An unlinked, authenticated audit view provides filtered pipeline/login totals, actor attribution, LLM latency and timeout metrics by call type, recent model calls, and an expandable diagnostic timeline. Bearer tokens, direct local requests, and caller-spoofed identity headers cannot retrieve shared audit data. |
...
| Operator dashboard authorization | `F4.6.8` | **Built** | Hosted dashboard access requires an individual UI account/session; knowing the unlinked route or holding an API Bearer token is insufficient. |
